    ...
    <div className="max-w-2xl mx-auto p-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Bug reports and feature requests you have submitted, and what we did
          with them.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center border border-dashed border-white/10 rounded-xl text-muted-foreground text-sm">
          Nothing yet. Hit the "Send feedback" button at the bottom-right to
          report a bug or suggest a feature.
        </div>
      ) : (
        <div className="space-y-3">
          {bugs.map((bug) => {
            const isFeature = bug.type === "FEATURE_REQUEST";
            const resolvedVerb = isFeature ? "Shipped" : "Fixed";
            return (
              <div
                key={bug.id}
