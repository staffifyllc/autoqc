...
export async function PATCH(
  ...
  try {
    ...
    // Notify the reporter when the bug is freshly marked FIXED.
    if (
      update.status === "FIXED" &&
      ...
      if (reporter?.email) {
        ...
        const html = `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111;background:#f4f5f7;padding:24px;">
          <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:24px;">
            <div style="display:inline-block;background:#10c76c;color:#fff;padding:6px 10px;border-radius:8px;font-size:13px;font-weight:700;margin-bottom:16px;">Fixed</div>
            <h2 style="margin:0 0 12px 0;font-size:18px;">Your bug report is live.</h2>
            ...
            <div style="margin:20px 0;padding:16px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;">
              <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;font-weight:600;margin-bottom:4px;">Your report</div>
            ...
            <p style="margin:20px 0 0 0;font-size:12px;color:#6b7280;">
              Reply to this email if the fix does not look right.
            </p>
          </div>
        </body></html>`;
        sendEmail({
          to: reporter.email,
          subject: `Fixed: ${existing.title.slice(0, 80)}`,
          html,
          ...
          console.error("reporter bug-fixed notify failed", err)
        );
      }
    }
